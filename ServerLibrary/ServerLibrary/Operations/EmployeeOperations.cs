...
{
    public static class EmployeeOperations // see IOperation
    {
        ...
        public static IQueryable<Account> TryList(Account requester, DataContext context)
        {
            ...
            return context.Accounts.Where(a => a.authz >= Account.AUTHZ_TECHNICIAN).AsQueryable<Account>();
        }
        ...
        public static void TryDelete(Account requester, DataContext context, int id)
        {
            ...
            // Customers -- delete NOT OK
            // Issues    -- delete NOT OK
            Customer customer = context.Customers.FirstOrDefault(c => id == c.technicianid || id == c.managementid);
            if (customer != null)
            {
                throw new ServerConflictException("Konto används som ansvarig för kund " + customer.name);
            }
