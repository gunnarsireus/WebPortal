...
$(function () {
    ...
    createPanel = $('#create-offer-panel');
    updateForm  = $('#update-offer-form');
    updatePanel = $('#update-offer-panel');
    ...
    itemsTable = $('#offer-table').dataTable({
    ...
    $('#offer-table tbody').on('click', 'td:not(.dataTables_empty)', function (e) {
        var row = $(this).parent();
        ...
        if (col == EDIT_CMD_INDEX) {
            siteAjaxReadToForm('/Offer/ReadOffer', { id: id }, updateForm, openUpdatePanel);
        }
            ...
            var warning  = (getCustomerSelection() !== '0') ?
                'Vill du ta bort erbjudande "' + headline + '" ?' :
                'OBS - detta påverkar alla kunder - vill du ta bort erbjudande "' + headline + '" ?';
            Site.Dialogs.confirm('/img/delete.png', 'Ta bort erbjudande', warning, 'Nej', 'Ja', function () {
                siteAjaxPost('/Offer/DeleteOffer', { id: id }, resetPage);
            });
        }
    });
    ...
    createForm.find('.create-offer-form-buttons').click(function () {
        var button = $(this);
        if (Site.InputPanels.isCLOSE(button)) {
            ...
            data && siteAjaxPost('/Offer/CreateOffer', data, resetPage);
        }
    });

    updateForm.find('.update-offer-form-buttons').click(function () {
        var button = $(this);
        if (Site.InputPanels.isCLOSE(button)) {
            closePanels();
        }
            ...
            data && siteAjaxPost('/Offer/UpdateOffer', data, resetPage);
        }
    });
});
